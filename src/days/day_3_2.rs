...
pub fn run() {
    ...
    println!(
        "[Day 3, Task 2] - The result of valid conditional memory calculations: {}",
        result
    )
}
...
fn validate_memory_from_file(memory: &str) -> i32 {
    ...
    while index < bytes.len() {
        if bytes[index..].starts_with(b"do()") {
            active = true;
            index += 4;
            continue;
        } else if bytes[index..].starts_with(b"don't()") {
            active = false;
            index += 7;
            continue;
        }
...
mod tests {
    ...
    fn test_validate_memory() {
        let memory_string = String::from(
            "xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))",
        );
        assert_eq!(validate_memory_from_file(&memory_string), 48);
    }
}
